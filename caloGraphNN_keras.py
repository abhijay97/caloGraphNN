...
from caloGraphNN import euclidean_squared, gauss, gauss_of_lin, gauss_root
...
class GarNet2(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        super(GarNet2, self).build(input_shape)
    ...
    def get_config(self):
            ...
            base_config = super(GarNet2, self).get_config()
            return dict(list(base_config.items()) + list(config.items()))
...
class GarNet(keras.layers.Layer):
    ...
    def call(self, x):
        features = self.input_feature_transform(x) # (B, V, F)
        distance = self.aggregator_distance(x) # (B, V, S)
        edge_weights = gauss_of_lin(distance)
        features = tf.concat([features, edge_weights], axis=-1) # (B, V, F+S)
        # vertices -> aggregators
        edge_weights_trans = tf.transpose(edge_weights, perm=(0, 2, 1)) # (B, S, V)
        aggregated_max = self.apply_edge_weights(features, edge_weights_trans, aggregation=tf.reduce_max) # (B, S, F+S)
...
class GarNet3(keras.layers.Layer):
    def __init__(self, n_aggregators, n_filters, n_propagate, name,  **kwargs):
        super(GarNet3, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        # tf.ragged FIXME? tf.shape()?
        self.output_feature_transform.build((input_shape[0], input_shape[1], input_shape[2] + self.aggregator_distance.units + 2 * self.aggregator_distance.units * self.aggregator_distance.units))

        for layer in self._sublayers:
            self._trainable_weights.extend(layer.trainable_weights)
            self._non_trainable_weights.extend(layer.non_trainable_weights)

        super(GarNet3, self).build(input_shape)
    ...
    def get_config(self):
            config = {'n_aggregators': self.n_aggregators, 'n_filters': self.n_filters, 'n_propagate': self.n_propagate, 'name': self.name}
            base_config = super(GarNet3, self).get_config()
            return dict(list(base_config.items()) + list(config.items()))
    # tf.ragged FIXME? the last one should be no problem


class GarNet4(keras.layers.Layer):
    def __init__(self, n_aggregators, n_filters, n_propagate, name,  **kwargs):
        super(GarNet4, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        # tf.ragged FIXME? tf.shape()?
        self.output_feature_transform.build((input_shape[0], input_shape[1], input_shape[2] + self.aggregator_distance.units + self.aggregator_distance.units * (self.input_feature_transform.units )))

        for layer in self._sublayers:
            self._trainable_weights.extend(layer.trainable_weights)
            self._non_trainable_weights.extend(layer.non_trainable_weights)

        super(GarNet4, self).build(input_shape)
    ...
    def call(self, x):
        ...
        # vertices -> aggregators
        edge_weights_trans = tf.transpose(edge_weights, perm=(0, 2, 1)) # (B, S, V)
        #aggregated_max = self.apply_edge_weights(features, edge_weights_trans, aggregation=tf.reduce_max) # (B, S, F)
        aggregated_mean = self.apply_edge_weights(features, edge_weights_trans, aggregation=tf.reduce_mean) # (B, S, F)

        #aggregated = tf.concat([aggregated_max, aggregated_mean], axis=-1) # (B, S, 2*(F+S))
        # aggregators -> vertices
        updated_features = self.apply_edge_weights(aggregated_mean, edge_weights) # (B, V, S*F)
        updated_features = tf.concat([x, updated_features, edge_weights], axis=-1) # (B, V, X+S*F+S)
    ...
    def get_config(self):
            config = {'n_aggregators': self.n_aggregators, 'n_filters': self.n_filters, 'n_propagate': self.n_propagate, 'name': self.name}
            base_config = super(GarNet4, self).get_config()
            return dict(list(base_config.items()) + list(config.items()))

class GarNet5(keras.layers.Layer):
    def __init__(self, n_aggregators, n_filters, n_propagate, name,  **kwargs):
        super(GarNet5, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        # tf.ragged FIXME? tf.shape()?
        self.output_feature_transform.build((input_shape[0], input_shape[1], self.aggregator_distance.units + self.aggregator_distance.units * (self.input_feature_transform.units )))

        for layer in self._sublayers:
            self._trainable_weights.extend(layer.trainable_weights)
            self._non_trainable_weights.extend(layer.non_trainable_weights)

        super(GarNet5, self).build(input_shape)
    ...
    def call(self, x):
        ...
        # aggregators -> vertices
        updated_features = self.apply_edge_weights(aggregated_mean, edge_weights) # (B, V, S*F)
        updated_features = tf.concat([updated_features, edge_weights], axis=-1) # (B, V, S*F+S)
    ...
    def get_config(self):
            config = {'n_aggregators': self.n_aggregators, 'n_filters': self.n_filters, 'n_propagate': self.n_propagate, 'name': self.name}
            base_config = super(GarNet5, self).get_config()
            return dict(list(base_config.items()) + list(config.items()))
